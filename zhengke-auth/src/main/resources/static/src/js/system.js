...
function switchClass(){
	var classes = ['skin-blue','skin-blue-light',
	...
	var names = ['蓝','亮蓝','黑','亮黑','绿','亮绿','红','亮红','黄','亮黄','紫','亮紫'];
	...
	$.each(classes,function(index,item){
		...
		if($("body").hasClass(item)){
			$("body").removeClass(item).addClass(classes[index+1]);
			return false;
		}
	});				
}
...
(function ($) {
    $.learuntab = {
        ...
        refreshTab: function () {
            ...
            var url = target.attr('src')+"1";//换新的连接，重新请求
            //$.loading(true);
        ...
        addTab: function () {
            ...
            if (flag) {
                ...
                var str1 = '<iframe class="LRADMS_iframe" id="iframe' + dataId + '" name="iframe' + dataId + '"  width="100%" height="100%" src="' + dataUrl+"?v="+Math.random() + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
        ...
        addJsOrCss:function(array,parentTag){
        	 $.each(array,function(index,item){
        	 	var tag ='';
        	 	if(item.endsWith(".js")){
        	 		tag = document.createElement("script");
        	 		tag.src = item+"?v" + Math.random();
        	 	}
        	 	if(item.endsWith(".css")){
        	 		tag = document.createElement("link");
        	 		tag.rel="stylesheet";
        	 		tag.href = item+"?v" + Math.random();
        	 	}
